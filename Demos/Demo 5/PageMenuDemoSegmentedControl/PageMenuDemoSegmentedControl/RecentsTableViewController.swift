...
class RecentsTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        // Configure the cell...
        cell.nameLabel.text = namesArray[(indexPath as NSIndexPath).row]
        cell.photoImageView.image = UIImage(named: photoNameArray[(indexPath as NSIndexPath).row])
        cell.dateLabel.text = dateArray[(indexPath as NSIndexPath).row] as! NSString as String
        cell.nameLabel.textColor = UIColor(red: 85.0/255.0, green: 85.0/255.0, blue: 85.0/255.0, alpha: 1.0)
        
        if activityTypeArray[(indexPath as NSIndexPath).row] as! Int == 0 {
            cell.activityImageView.image = UIImage(named: "phone_send")
        } else if activityTypeArray[(indexPath as NSIndexPath).row] as! Int == 1 {
